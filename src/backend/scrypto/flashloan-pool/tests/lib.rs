...
fn unit_test_deposit_lsu() -> Result<(), RuntimeError> {
    ...
    //  e.g. 1 supplier, 1000 LSU provided, and 0 rewards and interest
...
fn unit_test_withdraw_lsu() -> Result<(), RuntimeError> {
    ...
    assert_eq!(lsu_bucket.amount(&mut env)?, dec!("1000"));
...
fn integration_test_staker_deposit_lsu() {
    ...
    let mut amount: Decimal = dec!("501");
